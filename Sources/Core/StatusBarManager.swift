...
public class StatusBarManager: NSObject {
    ...
    public func createStatusIcon(for status: WorkflowStatus) -> NSImage {
        // Map internal status to shared factory status
        let factoryStatus: StatusIconFactory.WorkflowStatus
        switch status {
        case .unknown: factoryStatus = .unknown
        case .passing: factoryStatus = .passing
        case .failing: factoryStatus = .failing
        case .running: factoryStatus = .running
        case .runningAfterSuccess: factoryStatus = .runningAfterSuccess
        case .runningAfterFailure: factoryStatus = .runningAfterFailure
        }
        
        return StatusIconFactory.createStatusIcon(for: factoryStatus)
    }
    ...
    private func getTooltipText(for status: WorkflowStatus) -> String {
        ...
        case .runningAfterSuccess:
            return "Harbinger: Workflows running (last status: passing)"
        case .runningAfterFailure:
            return "Harbinger: Workflows running (last status: failing)"
        }
    }
