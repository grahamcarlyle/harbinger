...
final class GitHubClientTests: XCTestCase {
    ...
    func testGitHubClientInitialization() {
        XCTAssertNotNil(gitHubClient, "GitHub client should be initialized successfully")
    }
    ...
    // MARK: - Authenticated Tests (only run if token available)
    
    func testGetRepositoriesIfAuthenticated() throws {
        guard GitHubOAuthConfig.isConfigured else {
            throw XCTSkip("OAuth token not configured. Skipping authenticated test.")
        }
